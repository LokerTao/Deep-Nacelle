...
class Generator(nn.Module):
    def __init__(self, hidden_sizes,hidden_number,dropout_rate,activation_fn=F.leaky_relu,input_size=12, output_size=1):
        ...
        if activation_fn == F.leaky_relu:
            self.activation_fn = partial(activation_fn, negative_slope=0.2)
        else:
    ...
    def forward(self, x):
        ...
        for layer, bn_layer,dropout in zip(self.layers, self.bn_layers, self.dropouts):
            x = self.activation_fn(bn_layer(layer(x)))
            x = dropout(x)

        # Pass through output layer
        x = self.output_layer(x)
        x = x.view(-1, 1)  # Adjust the dimensions as per the output size
